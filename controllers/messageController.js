...
export const sendMessage = asyncHandler(async (req, res) => {
  ...
  const newMessage = {
    ...
    content: content || "",
    chat: chatId,
    readBy: [req.user._id], // Only sender has read the message
  ...
  try {
    ...
    // Unread Count Update - THIS PART NEEDS FIXING
    const otherParticipants = chat.participants.filter(
      (p) => p.toString() !== req.user._id.toString()
    );

    // Create an update object for the unreadCount map
    const updateObj = {};
    ...
    // Apply the update to the chat document
    await Chat.findByIdAndUpdate(chatId, { $set: updateObj });
